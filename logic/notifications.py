...
def create_notification(user_id, role, message, type="info", product_id=None, order_id=None, is_visible=True):
    ...
    Args:
        ...
        product_id (int or None): Optional related product ID
        order_id (int or None): Optional related order ID
        is_visible (bool): Should the notification be shown in the interface
    """
...
def get_user_notifications(role, user_id=None):
    """
    Return list of visible notifications for a user based on role and user_id.

    Args:
        role (str): 'admin', 'merchant', 'customer', 'visitor'
        user_id (int or None): if provided, returns both user-specific and public
